...
[cron.ts](../packages/scheduler/src/cron.ts) 是不依赖 DSH/数据库的五字段 cron 包装层：`validateCron` 和 `nextOccurrence` 只接受数字、列表、范围、步长、星期日 0/7 等 LoongBoard canonical 语法，并将时区、夏令时和 DOM/DOW 计算委派给 `cron-parser`。命名字段及其他扩展语法会被拒绝。春季跳时产生的不存在 wall time 遵循 `cron-parser` 的 transition adjustment；秋季回拨产生的重复 wall time 只执行一次，具体边界以 cron 测试为准。任务 CRUD、运行记录分别由 [HTTP routes](../apps/server/src/scheduled-tasks.ts) 和 [database service](../packages/database/src/scheduler-service.ts) 负责。

## 执行流程

[SchedulerEngine](../apps/server/src/scheduler.ts) 是唯一的 timer/run engine，使用 `Map<taskId, setTimeout>` 为每个任务维护最近一次 occurrence。
...
3. 每个 Agent run 都创建一个全新的持久 general session，原样发送保存的 prompt；run 记录保存该 `agentSessionId`，用户可从运行记录打开并继续聊天。System task 通过同一 engine 的 executor 执行九个 canonical system action，不创建 Agent 会话。
...
Repository sync run history 的 purge 不是普通同步路径，也不与 metadata archive 混用：固定删除 30 天前且不在最新 100 条内的 terminal runs，保护 queued/running 和 History 的 `last_run_id`；stream/target 子行随 parent cascade 删除。SQLite free pages 不会因每次 purge 自动 `VACUUM`；当前没有产品级数据库压缩 operation，文件回收不进入同步或维护热路径。

...
LoongBoard 的持久状态分布在配置文件、运行 state、Personal Data 仓库和可选的 Agent Archive。重大升级、迁移 data directory 或修改 storage 前，先停止 native Server 或 Docker container，确认没有写入，再备份。
...
- `system workspace/settings.json`、`domains/`、`prompts/`：控制中心设置、Domain 源文件和系统 update prompt；这里的 `prompts/` 不是 Personal Data 内的可移植 prompt 目录。
- `personalData.path`：完整 Git 仓库、`.git` 历史及其 `knowledge/`、`prompts/`、`skills/`。
...
停止服务后，将配置文件、YAML 指定的 state/Personal Data 路径和独立 archive repository 一并复制到备份介质。推荐使用保留权限和目录结构的归档工具：

```bash
tar -C "$BACKUP_SOURCE" -czf "$BACKUP_FILE" .
```

其中 `BACKUP_SOURCE` 必须是已经停止写入的 data root，`BACKUP_FILE` 必须位于 data root 之外。不要在服务运行时执行上述归档，也不要只备份 `knowledge.path` 来声称已经保留完整 Personal Data 或 Agent transcript。
...
3. 将备份恢复到目标 data root，保持 `system.yaml`、state、Personal Data 和 archive 的目录结构与权限。
...
恢复不会自动重新创建外部 GitHub 凭证、远端 repository 或 Docker volume 之外的本地 repository。不要运行 `git init` 覆盖已有 Personal Data 或 archive Git 历史；如果目标 Git 仓库缺失，应先恢复其完整 `.git` 目录或按部署流程重新导入。

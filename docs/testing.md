...
`tests/regression` is intentionally small and independent of the unit suites.
It currently protects only three high-impact
contracts:
...
- 当前 HTTP boundary 由 health/auth/sync-history/issue-detail focused tests 覆盖，runtime construction/projection tests 和 backend-critical regression tests 另有轻量覆盖；这些测试验证 production/test builder 分界、required SyncCoordinator route 调用及未改变的 HTTP path/schema。它们不等同于全量 `pnpm check`、真实 UI、Docker 或 live provider/DSH 验收。
...
- Repository retention settings and archive/run state are covered by Server/database tests; Web component tests cover Archive/All filters, Security 页面和标题重命名。布局、路由跳转、cookie 续期和交互可用性必须通过真实 browser acceptance。
...
functions and injected dependencies. Do not recreate broad suites, recording
fixtures, or a second implementation solely for tests.

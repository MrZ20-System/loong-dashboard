...
| `/repositories/:id/pulls`、`/issues` 及各自 `/activity-days` | GET 列表和日期活动；PR 支持 `updated` / `number` sort，使用 `page` + `limit` 页码分页；Issue 继续使用 cursor；status 支持重复 query 参数并按 ANY-match 过滤；PR/Issue 可选重复的 `archive=current|archived` 参数，省略时为 current，`archive=all` 仅保留兼容语义 | [metadata](../packages/contracts/src/metadata.ts) / [metadata route](../apps/server/src/routes/metadata.ts) |

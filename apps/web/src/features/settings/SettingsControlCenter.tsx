  ...
  return (
    <section className="settings-control-center" aria-labelledby="settings-control-heading">
      ...
      <div className="settings-control-grid">
        <article className="settings-control-tile"><span className="settings-tile-icon">↻</span><h3>{t({ en: "Repositories", "zh-CN": "仓库" })}</h3><p>{t({ en: "Sync status and Cron schedules for each configured repository.", "zh-CN": "查看每个已配置仓库的同步状态和 Cron 计划。" })}</p><Link to="/settings/repositories">{t({ en: "Manage repositories", "zh-CN": "管理仓库" })}</Link></article>

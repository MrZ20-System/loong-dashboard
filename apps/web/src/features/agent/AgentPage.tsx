...
import { deleteAgentSession, ensureAgentSession, listAgentSessions, updateAgentSession } from "../../agent-chat-client";
...
function sessionTitle(session: AgentSessionSummary): string {
  return session.title?.trim() || originLabel(session);
}
...
export function AgentPage() {
  ...
  const filtered = useMemo(() => (sessions.data?.items ?? []).filter((session) => {
    ...
    const text = `${session.id} ${sessionTitle(session)} ${originLabel(session)} ${session.scope.repositoryId ?? ""} ${session.model}`.toLowerCase();
  ...
  const rename = useMutation({
    mutationFn: ({ id, title }: { id: string; title: string }) => updateAgentSession(id, { title }),
    onSuccess: (view) => {
    ...
    onError: (failure: Error) => setRenameError(failure.message),
  });
                  ...
                  <div className="agent-session-row-wrap">
                    {isEditing ? (
                      ...
                      >
                        ...
                        />
                        <button type="submit" disabled={rename.isPending} aria-label="Save conversation title">Save</button>
                        <button type="button" onClick={() => { setEditingSessionId(null); setRenameError(null); }} aria-label="Cancel conversation rename">Cancel</button>
                      </form>
                    ) : (
                      <button type="button" className={session.id === selectedId ? "agent-session-row active" : "agent-session-row"} onClick={() => select(session)}>
                        <span className={`agent-row-status status-${session.status}`} aria-label={session.status} />
                        <span className="agent-session-row__copy">
                          <strong>{title}</strong>
                          <small>{originLabel(session)} · {session.scope.repositoryId ?? "Workspace"} · {new Date(session.lastUsedAt).toLocaleString()}</small>
                        </span>
                    ...
                    {!isEditing && (
                      <span className="agent-session-actions">
                        <button type="button" className="agent-session-rename-button" aria-label={`Rename ${title}`} onClick={() => startRename(session)}>✎</button>
                        <button type="button" className="agent-session-delete" aria-label={`Delete ${title}`} onClick={() => { if (window.confirm("Delete this conversation?")) remove.mutate({ id: session.id, scopeKey: JSON.stringify(session.scope) }); }}>×</button>
                      </span>
                    )}
                  </div>
                  {isEditing && renameError !== null && <p className="agent-session-rename-error" role="alert">{renameError}</p>}
                </li>
              );
            })}
          </ul>
        </aside>
        <div className="agent-page-surface">{selected ? <><div className="agent-page-source"><span>{sessionTitle(selected)} · {originLabel(selected)} · {selected.status}</span><Link to={sourceHref(selected)}>Open source</Link></div><AgentChatPanel scope={selected.scope} initialSessionId={selected.id} heading="Agent" panelId="agent-page-chat" showCollapseControl={false} /></> : <div className="agent-page-empty"><span className="settings-tile-icon">✦</span><h3>Select a conversation</h3><p>Choose an existing session to continue it with the same runtime context.</p></div>}</div>
      </div>
    </section>
  );
}

...
function MetadataTable({ kind, items }: { kind: "pulls" | "issues"; items: Array<PullRequestListItem | IssueListItem> }) {
  if (kind === "pulls") return <table><caption>Pull requests</caption><thead><tr><th scope="col">Number</th><th scope="col">Title</th><th scope="col">Author</th><th scope="col">Status</th><th scope="col">Domains</th><th scope="col">Updated</th><th scope="col">Files</th><th scope="col">Additions</th><th scope="col">Deletions</th></tr></thead><tbody>{(items as PullRequestListItem[]).map((item) => <tr key={item.number}><td>#{item.number}</td><td><a href={item.url} target="_blank" rel="noreferrer">{item.title}</a></td><td>{item.authorLogin ?? "Unknown"}</td><td>{item.status}</td><td><DomainChips domains={item.domains} /></td><td>{item.updatedAt}</td><td>{item.changedFilesCount}</td><td>{item.additions}</td><td>{item.deletions}</td></tr>)}</tbody></table>;
